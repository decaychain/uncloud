...
- **Audit log**: handlers that change a file or folder's existence, name, location, content, or trash status must call `audit::file_event` / `audit::folder_event` (or `summary_event` for bulk ops) — emits a `SyncEvent` row and broadcasts `ServerEvent::SyncEventAppended`. Out of scope: pure folder-settings changes (sync strategy, gallery/music inclusion), access-control changes (folder shares), and non-file domains (tasks, shopping, playlists, music categories, etc.) — they emit their own `ServerEvent` variants instead.
