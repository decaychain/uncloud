...
object NativeAudioRuntime {
    ...
    private val playerListener = object : Player.Listener {
        ...
        override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
            // Fires when ExoPlayer's native playlist advances to a new item.
            // Emit so the WASM side can mirror current_index without running a
            // JS timer — important during Doze when the WebView is suspended.
            emitState()
        }
    }
    ...
    fun setQueue(
        ...
        synchronized(lock) {
            ...
            val mediaItems = items
                .mapNotNull { it.src?.takeIf { s -> s.isNotBlank() }?.let { s -> buildMediaItem(s, it.title, it.artist, it.artworkUrl) } }
            if (mediaItems.isEmpty()) return

            val safeStart = startIndex.coerceIn(0, mediaItems.size - 1)
            val safePositionMs = if (startPositionMs >= 0) startPositionMs else 0L

            pendingSeekState = null
            currentStoryId = null
            exoPlayer.setMediaItems(mediaItems, safeStart, safePositionMs)
            exoPlayer.prepare()
            lastError = null
            syncTickingLocked()
        }
    ...
    fun seekToItem(context: Context, index: Int) {
        synchronized(lock) {
            ensure(context)
            val exoPlayer = player ?: return
    ...
    fun setRepeatMode(context: Context, mode: String) {
        val repeat = when (mode.lowercase()) {
...
class NativeAudioPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    fun setQueue(invoke: Invoke) {
        ...
        val items = args.items.orEmpty()
        if (items.isEmpty()) {
            invoke.reject("items is required")
            return
        }
        val startIndex = args.startIndex ?: 0
        val startPositionMs = args.startPosition?.let { if (it.isFinite()) (it * 1000.0).toLong() else 0L } ?: 0L

        runCatching {
            NativeAudioRuntime.setQueue(activity.applicationContext, items, startIndex, startPositionMs)
        }.onSuccess {
            invoke.resolve(toJsObject(NativeAudioRuntime.getState(activity.applicationContext)))
        }.onFailure {
    ...
    @Command
    fun seekToItem(invoke: Invoke) {
        val args = invoke.parseArgs(SeekToItemArgs::class.java)
        ...
        runCatching {
            NativeAudioRuntime.seekToItem(activity.applicationContext, index)
        }.onSuccess {
            invoke.resolve(toJsObject(NativeAudioRuntime.getState(activity.applicationContext)))
        }.onFailure {
            invoke.reject(it.message ?: "seekToItem failed")
        }
    }
    ...
    fun setRepeatMode(invoke: Invoke) {
        ...
        val mode = args.mode?.trim().orEmpty()
        runCatching {
    ...
    private fun toJsObject(state: NativeAudioState): JSObject {
        ...
        payload.put("currentIndex", state.currentIndex)
