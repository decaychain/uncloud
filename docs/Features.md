...
- **Feature gate**: server-wide `features.music`; users can also opt out in Settings. The gate covers both `/api/music/*` and `/api/playlists/*`.
...
- **Music Categories**: user labels attached to one or more music folders. The Library view shows a category dropdown + live filter; categorized folders are the only folders shown in the Music sidebar and open the library scoped to that folder's subtree. Categories are managed from a folder's "⋮ → Manage categories…" menu.
- **Frontend**:
  - Music page with Library and Folders view modes. Library handles search + category filtering; Folders renders a lazy, expand-on-demand folder tree in the main content area and opens track lists inline. `/music/folder/:id` uses the same browser rooted at that folder.
  ...
  - Components in `components/music/`: `artist_list`, `artist_view`, `album_grid`, `album_view`, `folder_tree`, `playlist_list`, `playlist_view`, `playlist_panel` (persistent right-side), `track_list`, `manage_categories`
...
A self-hosted personal expense tracker — multi-account, multi-currency (per-currency totals, no FX conversion), with idempotent CSV import, user-editable import schemas, undoable import runs, IBAN-driven account auto-creation, reconciliation snapshots, a categorization-rules engine, and informal-IOU settlements.

- **Model** (MongoDB collections; all nine backed up):
  - `finance_accounts` — `FinanceAccount { owner_id, name, account_type, currency, opening_balance_minor, iban, archived_at }`. `iban` is normalized (whitespace stripped, uppercased) with a partial unique index per `(owner_id, iban)`.
  - `finance_categories` — `FinanceCategory { owner_id, parent_id, name, colour }`.
  - `finance_transactions` — `FinanceTransaction { owner_id, account_id, currency, amount_minor, description, date, source_ref, raw_bank_category, notes, tags, legs, import_run_id, source_snapshot_id }`. The two new ids tag rows so an import run can be reverted in one query and reconciliation adjustments can be filtered out from the default listing.
  - `finance_import_schemas` — `ImportSchema { … delimiter, encoding, decimal_separator, date/amount/iban/raw_category columns, currency_source, is_builtin, builtin_id }`. The Sparkasse CAMT V8 template is seeded as a builtin on first list; users clone it to get an editable copy.
  - `finance_import_runs` — `ImportRun { schema_id, source { kind: upload | uncloud_file, filename, size_bytes, sha256, uncloud_file_id? }, status: applied | reverted, summary { created, skipped_duplicate, errored }, errors }`. Reverting deletes every transaction tagged with the run's id; the run row stays as an audit trail.
  - `finance_balance_snapshots` — `BalanceSnapshot { account_id, on_date, actual_balance_minor, note, adjustment_transaction_id }`. Backs the Reconcile flow; drift against later imports is detected on read.
  - `finance_rules` — `FinanceRule { name, pattern, pattern_kind: substring | starts_with | wildcard | regex, case_insensitive, category_id, priority, enabled }`. Applied during import (first match wins, ordered by priority) and on demand via `POST /finance/rules/apply`. Never overwrites legs whose `category_source = user`.
  - `finance_settlements` — `FinanceSettlement { owner_id, counterparty, direction: owed_to_me | owed_by_me, amount_minor, currency, category_id, description, notes, opened_at, next_payment_at, source_transaction_id, status: open | settled | forgiven, closed_at }`.
  - `finance_settlement_entries` — `SettlementEntry { owner_id, settlement_id, kind: payment | forgiveness | charge, counterparty (per-entry override for group settlements), amount_minor, date, linked_transaction_id, note }`. Entries are the source of truth: totals are aggregated from this collection at read time (outstanding = amount + charges − payments − forgiveness) and status is recomputed on every entry write. A `charge` may land on a settled settlement and reopens it; deleting a settlement cascades to its entries.
...
- **Rules engine**: `crates/uncloud-server/src/finance_rules.rs` compiles `FinanceRule`s into a `RuleEngine` (substring, prefix, wildcard, and regex via the `regex` crate — linear-time). Disabled rules are skipped; bad patterns are rejected at write time. `POST /finance/rules/test` returns matches from the last 200 transactions without writing — used by the editor's live-preview "Run test" button.
- **API surface** (all under `/api/finance/`):
  - `accounts` (CRUD + balance), `categories` (CRUD), `transactions` (CRUD + `category-summary` aggregation),
  - `import-schemas` (CRUD + `/{id}/clone`), `import` (commit), `imports` (list runs + `/{id}/revert`),
  - `accounts/{id}/reconcile/{preview,apply}`, `accounts/{id}/snapshots`, `snapshots/{id}` (delete + `/recompute`),
  - `rules` (CRUD), `rules/apply`, `rules/test`,
  - `settlements` (CRUD + GET `/{id}` detail with entries; list filterable by `status`, `category_id` incl. descendants, `uncategorized`), `settlements/{id}/entries` (create), `settlements/{id}/entries/{entry_id}` (delete) — entry mutations return the full detail response. Optional fields clear via empty string (JSON `null` cannot round-trip the two-level `Option`).
- **Frontend**: `components/finance.rs`. The sidebar shows seven sub-routes under Finance — **Transactions**, **Accounts**, **Categories**, **Settlements**, **Import** (with nested **Schemas**), and **Rules**. The Transactions view spans full width, supports a date range with presets (this/last month, last 30 days, this/last year, all time), a "Show reconciliations" toggle, optional desktop-only income/expenses split, IntersectionObserver-driven auto-load-more, and a collapsible summary strip above the table (per-account balance cards + Income/Expenses/Net totals for the range + per-category horizontal bars). The Rules editor has a live "Run test" preview and a "Save & apply rules" button that runs the engine over existing transactions and reports the count before closing. Each transaction row exposes a `⋮` menu with Edit / Create rule / Delete; "Create rule" pre-fills the editor with the transaction's description and category. The Settlements view is a full-width two-pane master/detail layout on desktop (1/3 list, 2/3 detail; separate screens on mobile via `/finance/settlements` and `/finance/settlements/:id`): list cards carry status badge, category badge, overdue-highlighted next-payment chip, and a slim fulfillment bar; the detail pane shows totals (original/charges/paid/forgiven/outstanding), notes, the entry timeline, and Add payment / Forgive / Add charge actions. The settlement form supports inline category creation and clearing the next-payment date.
...
- **Folders**: stores IMAP folder paths, hierarchy delimiters, parent paths, selectable state, role mapping, attributes, and per-folder sync state so subfolders can be represented without flattening. Folder refresh upserts by remote path to preserve stable ids and preserves manual role overrides.
- **Provider layer**: `MailService` currently uses `async-imap` for IMAP connection tests, folder refresh, bounded read-only summary sync, and on-demand body fetch. Sync initializes from the newest UID window, then prioritizes newly arrived mail before backfilling older UID windows. `lettre` handles SMTP connection/auth tests. `mail-parser` parses fetched MIME bodies and `ammonia` sanitizes HTML before it reaches the UI.
- **Credentials**: IMAP app passwords can be stored encrypted-at-rest with `secrets.master_key`. IMAP/SMTP connection checks, folder refresh, and manual sync accept either a transient password body or the stored account credential. OAuth refresh tokens and SMTP-specific credential splits are still future work.
- **Message cache and sync**: manual or scheduled sync stores envelope, flags, internal date, RFC822 size, and address metadata in `mail_messages`. Scheduled sync runs only for accounts with stored credentials and automatic sync enabled, using the same bounded latest-first strategy, a configurable per-account interval, and a per-account in-memory lock. The reader can fetch a body on demand and display sanitized HTML with remote image URLs stripped, plus a plain-text fallback.
- **API surface**: `/api/mail/accounts`, `/api/mail/accounts/{id}/credential`, `/api/mail/accounts/{id}/test-imap`, `/api/mail/accounts/{id}/test-smtp`, `/api/mail/accounts/{id}/sync`, `/api/mail/accounts/{account_id}/folders`, `/api/mail/accounts/{account_id}/folders/refresh`, `/api/mail/accounts/{account_id}/folders/{folder_id}`, `/api/mail/accounts/{account_id}/folders/{folder_id}/sync`, `/api/mail/accounts/{account_id}/folders/{folder_id}/messages`, `/api/mail/messages/{message_id}`, and `/api/mail/identities`.
- **Frontend**: read-only `/mail` shell with account setup/settings, IMAP/SMTP tests, folder refresh/sync, per-folder settings, cached message list, and sanitized body reader.
