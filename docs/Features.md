...
- **Folders**: stores IMAP folder paths, hierarchy delimiters, parent paths, selectable state, attributes, and per-folder sync state so subfolders can be represented without flattening. Folder refresh upserts by remote path to preserve stable ids.
- **Provider layer**: `MailService` currently uses `async-imap` for IMAP connection tests, folder refresh, and bounded read-only summary sync. Sync initializes from the newest UID window, then prioritizes newly arrived mail before backfilling older UID windows. `lettre` handles SMTP connection/auth tests. `mail-parser` and `ammonia` are included as the planned MIME and HTML-sanitization foundation.
...
- **Message cache**: manual sync stores envelope, flags, internal date, RFC822 size, and address metadata in `mail_messages`; raw RFC822 bodies and attachments are still future work. The reader can fetch a body on demand and display crude plain text.
- **API surface**: `/api/mail/accounts`, `/api/mail/accounts/{id}/credential`, `/api/mail/accounts/{id}/test-imap`, `/api/mail/accounts/{id}/test-smtp`, `/api/mail/accounts/{id}/sync`, `/api/mail/accounts/{account_id}/folders`, `/api/mail/accounts/{account_id}/folders/refresh`, `/api/mail/accounts/{account_id}/folders/{folder_id}/sync`, `/api/mail/accounts/{account_id}/folders/{folder_id}/messages`, `/api/mail/messages/{message_id}`, and `/api/mail/identities`.
- **Frontend**: read-only `/mail` shell with account setup, IMAP/SMTP tests, folder refresh/sync, cached message list, and a placeholder body reader.
