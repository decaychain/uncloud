...
- **Model** (MongoDB collections; all seven backed up):
  ...
  - `finance_rules` — `FinanceRule { name, pattern, pattern_kind: substring | starts_with | regex, case_insensitive, category_id, priority, enabled }`. Applied during import (first match wins, ordered by priority) and on demand via `POST /finance/rules/apply`. Never overwrites legs whose `category_source = user`.
...
- **Rules engine**: `crates/uncloud-server/src/finance_rules.rs` compiles `FinanceRule`s into a `RuleEngine` (regex via the `regex` crate — linear-time). Disabled rules are skipped; bad regexes are rejected at write time. `POST /finance/rules/test` returns matches from the last 200 transactions without writing — used by the editor's live-preview "Run test" button.
