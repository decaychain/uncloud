...
- **Message cache**: manual sync stores envelope, flags, internal date, RFC822 size, and address metadata in `mail_messages`; raw RFC822 bodies and attachments are still future work. The reader can fetch a body on demand and display sanitized HTML with remote image URLs stripped, plus a plain-text fallback.
