...
A self-hosted personal expense tracker — multi-account, multi-currency (per-currency totals, no FX conversion), with idempotent CSV import, user-editable import schemas, undoable import runs, IBAN-driven account auto-creation, reconciliation snapshots, and a categorization-rules engine.

- **Model** (MongoDB collections; all seven backed up):
...
- **Message cache and sync**: manual or scheduled sync stores envelope, flags, internal date, RFC822 size, and address metadata in `mail_messages`. Scheduled sync runs only for enabled accounts with stored credentials and account sync enabled, using the same bounded latest-first strategy, a configurable per-account interval, and a per-account in-memory lock. The reader can fetch a body on demand and display sanitized HTML with remote image URLs stripped, plus a plain-text fallback.
