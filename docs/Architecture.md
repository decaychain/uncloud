...
- **Audit log**: handlers that change a file or folder's existence, name, location, content, or trash status call `audit::file_event` / `audit::folder_event` (or `summary_event` for bulk ops) to append a `SyncEvent` row (capped + TTL-purged); the row is also broadcast as `ServerEvent::SyncEventAppended` so other devices for the same user see it live. Scope is deliberately narrow — covers `Created`, `Renamed`, `Moved`, `Copied`, `ContentReplaced`, `Deleted`, `Restored`, `PermanentlyDeleted`. Explicitly **out of scope**: pure folder-settings changes (`sync_strategy`, `gallery_include`, `music_include` — `ServerEvent::FolderUpdated` covers those); folder-share lifecycle (`FolderShared` / `FolderShareRevoked`); and mutations in non-file domains like tasks, shopping, playlists, music categories — each has its own `ServerEvent` variant where cross-device coherence is needed.
