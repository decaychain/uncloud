...
- **Events**: SSE stream at `/api/events` for real-time updates. `EventService` broadcasts to per-user channels. Frontend subscribes via `use_events` hook (accepts `FnMut`, uses `Rc<RefCell<F>>`). The `ServerEvent` enum covers file/folder CRUD, processing, folder-share lifecycle, rescan progress, and sync-audit appends.
